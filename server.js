...
app.get("/userlangs/:prefix", function(request, response) {
  ...
  if (codestonames[prefix] && codestonames[prefix].length > 0) {
    let tempData = (typeof codestonames[prefix] === "string") ? [{
        text: codestonames[prefix],
        id: prefix
      }] :
      codestonames[prefix].map(function(x) {
        return {
          text: x,
          id: prefix
        }
...
app.get("/langnames/:code", function(request, response) {
  ...
  fs.readFile("langnames/" + request.params["code"] + ".json", function(err, data) {
    if (err)
      ...
      response.send(JSON.stringify(Object.keys(langdata).map(function(x) {
        return {
          id: x,
          text: langdata[x].toProperCase()
        }
      }), null, 2))
    }
  })
...
app.get("/territories/:code", function(request, response) {
  ...
  fs.readFile("territories/" + request.params["code"] + ".json", function(err, data) {
    if (err)
      ...
      const terrdata = arrayUnion((loc.country && rawdata[loc.country]) ? [{
          id: loc.country,
          text: rawdata[loc.country]
        }] : [],
        Object.keys(rawdata).sort().filter((x) => x !== loc.country).map(function(x) {
          return {
            id: x,
            text: rawdata[x].toProperCase()
          }
        }))
...
app.get("/userlangs", function(request, response) {
  ...
  response.send(getLangs(request).map(function(x) {
    return {
      text: codestonames[x].map((x) => x.toProperCase()),
      id: x
    }
  }))
})
...
app.get("/engname/:code", function(request, response) {
  ...
  if (engNames[code]) {
    if (typeof engNames[code] === "string")
      response.send(engNames[code])
    else
      response.send(engNames[code][0])
  } else if (codestonames[code]) {
    if (typeof codestonames[code] === "string")
      response.send(codestonames[code])
    else
      response.send(codestonames[code][0])
  } else {
    response.send("ERROR: LANGUAGE NOT FOUND")
  }
})
